 ...
 * 새 채팅 파일 생성
 */
async function createNewChatFile(newFileName) {
    const ctx = getContext();
    const currentCharId = ctx.characterId;
    ...
    // 현재 채팅 파일 ID 저장
    const originalChatFileId = removeJsonlExtension(currentCharacter.chat);
    
    try {
        // 새 채팅 시작 (SillyTavern의 doNewChat 기능 사용)
        await executeSlashCommands('/newchat');
        await sleep(2000);
        await waitForChatLoad();
        
        // 새 채팅 파일 ID 가져오기
        const newChatFileId = removeJsonlExtension(ctx.characters[currentCharId].chat);
        
        // 채팅 이름 변경 (rename 명령어 사용)
        if (newFileName && newFileName.trim()) {
            await executeSlashCommands(`/rename chat="${newFileName.trim()}"`);
            await sleep(500);
        }
        
        console.log('[Broadcast] New chat created:', newChatFileId);
        
        return {
            newFileId: newFileName.trim() || newChatFileId,
            originalFileId: originalChatFileId
        };
        
    } catch (error) {
        console.error('[Broadcast] Error creating new chat:', error);
        throw error;
    }
}

/**
 * 백업 대상 채팅 파일 선택 - API로 목록 가져오기 + 새 파일 생성 옵션
 */
